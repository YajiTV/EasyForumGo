...
func (r *PostRepository) GetByID(id string) (*model.Post, error) {
	var p model.Post
	err := r.db.QueryRow(`SELECT id, user_id, title, content, image_path, approved, created_at, updated_at FROM posts WHERE id = ?`, id).
		Scan(&p.ID, &p.UserID, &p.Title, &p.Content, &p.ImagePath, &p.Approved, &p.CreatedAt, &p.UpdatedAt)
	if err != nil {
		return nil, err
	}
	return &p, nil
...
func (r *PostRepository) GetAll() ([]model.Post, error) {
	rows, err := r.db.Query(`SELECT id, user_id, title, content, image_path, approved, created_at, updated_at FROM posts ORDER BY created_at DESC`)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	...
	for rows.Next() {
		var p model.Post
		if err := rows.Scan(&p.ID, &p.UserID, &p.Title, &p.Content, &p.ImagePath, &p.Approved, &p.CreatedAt, &p.UpdatedAt); err != nil {
			return nil, err
		}
...
// GetAllPaginated gets a page of approved posts
func (r *PostRepository) GetAllPaginated(limit, offset int) ([]model.Post, error) {
	return r.queryPosts(`SELECT id, user_id, title, content, image_path, approved, created_at, updated_at
		FROM posts WHERE approved = 1 ORDER BY created_at DESC LIMIT ? OFFSET ?`, limit, max(offset, 0))
}

// GetByUserID gets stored data (all posts, approved or not — author sees own)
func (r *PostRepository) GetByUserID(userID string) ([]model.Post, error) {
	rows, err := r.db.Query(`SELECT id, user_id, title, content, image_path, approved, created_at, updated_at FROM posts WHERE user_id = ? ORDER BY created_at DESC`, userID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
// GetByUserIDPaginated gets a page of a user's posts (all, approved or not — author sees own)
...
// GetFollowing gets a page of approved posts from followed users
func (r *PostRepository) GetFollowing(userID string, limit, offset int) ([]model.Post, error) {
	return r.queryPosts(`SELECT p.id, p.user_id, p.title, p.content, p.image_path, p.approved, p.created_at, p.updated_at
		FROM posts p JOIN user_follows f ON f.followed_id = p.user_id
		WHERE f.follower_id = ? AND p.approved = 1 ORDER BY p.created_at DESC LIMIT ? OFFSET ?`, userID, limit, max(offset, 0))
}
...
func (r *PostRepository) GetByCategory(categoryID string) ([]model.Post, error) {
	rows, err := r.db.Query(`SELECT p.id, p.user_id, p.title, p.content, p.image_path, p.approved, p.created_at, p.updated_at FROM posts p JOIN post_categories pc ON p.id = pc.post_id WHERE pc.category_id = ? ORDER BY p.created_at DESC`, categoryID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
// GetByCategoryPaginated gets a page of approved posts in a category
func (r *PostRepository) GetByCategoryPaginated(categoryID string, limit, offset int) ([]model.Post, error) {
	return r.queryPosts(`SELECT p.id, p.user_id, p.title, p.content, p.image_path, p.approved, p.created_at, p.updated_at
		...
		WHERE pc.category_id = ? AND p.approved = 1 ORDER BY p.created_at DESC LIMIT ? OFFSET ?`, categoryID, limit, max(offset, 0))
}
...
func (r *PostRepository) Create(p *model.Post) error {
	_, err := r.db.Exec(`INSERT INTO posts (id, user_id, title, content, image_path, approved, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
		p.ID, p.UserID, p.Title, p.Content, p.ImagePath, p.Approved, p.CreatedAt, p.UpdatedAt)
	return err
}
...
func (r *PostRepository) CreateWithCategories(p *model.Post, categoryIDs []string) error {
	...
	if _, err := tx.Exec(`INSERT INTO posts (id, user_id, title, content, image_path, approved, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
		p.ID, p.UserID, p.Title, p.Content, p.ImagePath, p.Approved, p.CreatedAt, p.UpdatedAt); err != nil {
...
func (r *PostRepository) queryPosts(query string, args ...any) ([]model.Post, error) {
	...
	for rows.Next() {
		var post model.Post
		if err := rows.Scan(&post.ID, &post.UserID, &post.Title, &post.Content, &post.ImagePath, &post.Approved, &post.CreatedAt, &post.UpdatedAt); err != nil {
			return nil, err
		}
...
func (r *PostRepository) Approve(id string) error {
	_, err := r.db.Exec(`UPDATE posts SET approved = 1 WHERE id = ?`, id)
	return err
}
